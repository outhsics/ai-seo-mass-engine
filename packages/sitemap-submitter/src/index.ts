...
interface SitemapSubmitConfig {
  siteUrl: string;
  sitemapUrl: string;
  credentialsPath: string; // Google Service Account JSON
}
...
export class SitemapSubmitter {
  private config: SitemapSubmitConfig;
  private jwt: JWT;
...
export class BatchSitemapSubmitter {
  private configs: SitemapSubmitConfig[];
  ...
  async submitAll(): Promise<SubmitResult[]> {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (process.env.SITE_URL && process.env.SITEMAP_URL) {
    const config: SitemapSubmitConfig = {
  ...
  // 批量提交（从配置文件读取）
    ...
    const configs: SitemapSubmitConfig[] = JSON.parse(readFileSync(configPath, 'utf-8'));
